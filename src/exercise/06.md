...
useRef is interesting, basically you set a variable equal to React.useRef(), then pass that variable to the element's ref property. You can then access the current state of that element by calling the .current property on the variable you created.

useState is simple, returns an array which you can destructure to get your reference to the state's value as well as a setter method, e.g.:
const [username, setUsername] = React.useState('');

the argument you pass to useState is the default value

React gives a syntheticEvent class instance which is super similar to a regular event object, it just has some stuff for form validation etc

To access form elements by their name or id, you can use event.target.elements.exampleIdName
