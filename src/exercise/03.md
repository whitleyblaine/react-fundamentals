...
Babel just compiles JSX into React.createElement calls. Get good at converting it in your head, it'll make you a more effective programmer.

You can pass a `props` object to JSX by using the spread operator:
```
...
A JavaScript Expression is any valid unit of code that resolves to a value. You can put any javascript expression inside the curly braces in JSX.
...
With spread operator, POSITION MATTERS. What comes later overrides what comes first.
```
