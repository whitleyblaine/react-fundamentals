...
The DOM is basically an object-oriented representation of the document, and is created when the browser parses the HTML generated on the server. Then it can be changed dynamically by interacting with the multiple APIs and interfaces that make up the DOM. One could theoretically use any programming language to do this, not just JavaScript, though JS is most common on the web.
