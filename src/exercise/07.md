...
If you return the same element types, it keeps those components/DOM nodes around, even if all the props changed.
*Important exception: if the KEY prop changes, React will re-render your component (unmount and mount a new one). 
This means all state is removed and the lifecycle methods are run.

IMPORTANT: Without a `key` prop, React defaults to using the index of the array to determine which React elements to unmount and which to modify.
This article was SUPER helpful in explaining this: https://epicreact.dev/why-react-needs-a-key-prop/

Basically, when React re-renders a component, it's diffing the current mounted component against the new component, and changing to match the diff.
But it can't do a granular enough diff without the `key` prop to know WHICH elements are intended to be changed, and which are intended to be unmounted.
The `key` prop makes it clear which elements are which during the diff.
